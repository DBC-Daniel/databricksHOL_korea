...
# user email address(top-right) > User Settings >  Access tokens > Generate new token
import os
os.environ["DATABRICKS_TOKEN"] = "TOKEN_"

# COMMAND ----------

df_white = spark.read.option("inferSchema", "true").csv("/databricks-datasets/wine-quality/winequality-white.csv", sep=";", header="true")
white_wine = df_white.toPandas()
df_red = spark.read.option("inferSchema", "true").csv("/databricks-datasets/wine-quality/winequality-red.csv", sep=";", header="true")
red_wine = df_red.toPandas()
...
model_name = "wine_quality"+"_"+databricks_user
...
def score_model(dataset):
  url = 'https://megazone-dbc-handson-azure-team.cloud.databricks.com/model/wine_quality_hyojoung_shin/3/invocations'
...
# MAGIC %md
# MAGIC 
# MAGIC 모델 서빙을 위해 생성한 클러스터를 꼭 삭제해주세요.
